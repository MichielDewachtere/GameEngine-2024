...
void LabScene::Load()
{
...

	{
		...
		auto& gamePadControls = CreateGameObject();
		{
			...
			gamePadControls.AddComponent<TextComponent>("Use the D-Pad to move Sno-Bee, Kill Pengo for points, press triangle/y to die", std::move(fontSmall), SDL_Color{ 255,255,255,255 });
		...
		// Lives Display
		{
			...
			livesDisplay.AddComponent<TextComponent>("test", std::move(fontSmall), SDL_Color{ 255,255,255,255 });
		...
		{
			auto fontSmall = ResourceManager::GetInstance().LoadFont("Lingua.otf", 16);
			keyboardControls.AddComponent<TextComponent>("Use WASD to move Pengo, press 1 for points, run into sno-bee to die", std::move(fontSmall), SDL_Color{ 255,255,255,255 });
