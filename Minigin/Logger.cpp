...
std::map<dae::LogLevel, std::string> dae::Logger::m_LevelToStr = {
		{LogLevel::debug, "DEBUG"},
		{LogLevel::info, "INFO"},
		{LogLevel::warning, "WARNING"},
		{LogLevel::error, "ERROR"} };

std::map<dae::LogLevel, std::string> dae::Logger::m_LevelToConsoleStyle = {
		{LogLevel::debug, "\033[32m"},
		{LogLevel::info, "\033[36m"},
		{LogLevel::warning, "\033[33m"},
		{LogLevel::error, "\033[31m"} };

void dae::Logger::Initialize()
{
#ifdef _DEBUG
...
void dae::Logger::ClearConsole()
{
	std::system("cls");
}

void dae::Logger::StartFileLogging(const std::string& filePath)
{
...
void dae::Logger::StopFileLogging()
{
	const auto ptr = m_pFileLogger.release();
	delete ptr;
}

bool dae::Logger::ProcessLog(LogLevel level, const LogString& log, std::format_args args)
{
#ifndef _DEBUG
...
std::string dae::Logger::GetTimeAndDate()
{
	const auto now = std::chrono::system_clock::now();
	const std::time_t time = std::chrono::system_clock::to_time_t(now);
